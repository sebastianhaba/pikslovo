...
public sealed class TranslationOrchestrator
{
        ...
        CancellationToken cancellationToken)
    {
        if (!settings.IsValid)
        {
            throw new TranslationException(AppStrings.Get(AppStrings.Keys.FillApiKeyAndLanguages));
        }
        ...
        try
        {
            ...
            {
                throw new TranslationException(AppStrings.Get(AppStrings.Keys.GoogleTranslationIncomplete));
            }
